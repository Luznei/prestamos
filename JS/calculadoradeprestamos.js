document.getElementById("calcular").addEventListener("click", function() {
    ...
    resultadoElement.innerHTML = "Cuota mensual estimada: <strong>$" + cuota.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "</strong><br>" +
                                 "Total del interes pagado: <strong>$" + totalInteresPagado.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "</strong>";
